...
if __name__ == '__main__':
    ...
    for x in range(total_episodes):
        ...
        if (x > 0) and (x % 5 == 0):
            ...
            plotter.plot(env)
            for state in qlearn.q:
                best_policy = {}
                best_action = max(qlearn.actions, key=lambda action: qlearn.q.get((state, action), 0))  # Get the action with max Q-value for the state
                best_policy[state] = best_action
            with open("Best_Policy.pickle", "wb") as f:
